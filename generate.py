#!/usr/bin/env python3
"""
Generate prospect preview files for estate-planning-form outreach.
Creates:
  prospects/form_configs.json      — brand config array (~85 unique firms)
  prospects/previews/{slug}.html   — standalone preview per firm
"""
import json, os, re

# Known Google Fonts: normalized name → Google Fonts URL family name
GOOGLE_FONTS = {
    'poppins': 'Poppins', 'poppins-regular': 'Poppins',
    'lato': 'Lato', 'montserrat': 'Montserrat', 'montserratregular': 'Montserrat',
    'roboto': 'Roboto', 'inter': 'Inter',
    'playfair': 'Playfair+Display', 'playfair display': 'Playfair+Display',
    'open sans': 'Open+Sans',
    'libre baskerville': 'Libre+Baskerville',
    'dm serif display': 'DM+Serif+Display',
    'cormorant garamond': 'Cormorant+Garamond',
    'source serif 4': 'Source+Serif+4',
    'rufina': 'Rufina', 'martel': 'Martel', 'newsreader': 'Newsreader',
    'petrona': 'Petrona', 'muli': 'Mulish', 'mulish': 'Mulish',
    'space grotesk': 'Space+Grotesk',
    'plus jakarta sans': 'Plus+Jakarta+Sans',
    'italiana': 'Italiana', 'public sans': 'Public+Sans',
    'crimson': 'Crimson+Text', 'crimson text': 'Crimson+Text',
    'rajdhani': 'Rajdhani', 'prata': 'Prata',
    'josefin sans': 'Josefin+Sans', 'eb garamond': 'EB+Garamond',
    'merriweather': 'Merriweather', 'nunito': 'Nunito',
    'raleway': 'Raleway', 'oswald': 'Oswald',
    'forum': 'Forum', 'dancing script': 'Dancing+Script',
}

def resolve_font(raw_font):
    """Return (css_font_stack, google_font_url_name_or_None)."""
    if not raw_font:
        return "'Comfortaa', sans-serif", None
    key = raw_font.strip().lower()
    gf = GOOGLE_FONTS.get(key)
    if gf:
        display_name = gf.replace('+', ' ')
        return f"'{display_name}', sans-serif", gf
    return "'Comfortaa', sans-serif", None

def clean_logo_url(url):
    """Return None for data: URIs or empty strings; otherwise pass through."""
    if not url:
        return None
...
INPUT_JSON  = '/Users/jaidhingra/Downloads/law_firm_brand_data_v2.json'
...
def get_colors(firm):
    p  = firm.get('primary_color') or ''
    pd = firm.get('primary_color_dark') or ''
    pc = firm.get('primary_color_light') or ''
    fallback = (not p or p == 'unknown')
    if fallback:
        return FALLBACK_PRIMARY, FALLBACK_DARK, FALLBACK_CONTAINER, True
    if not pd or pd == 'unknown': pd = FALLBACK_DARK
    if not pc or pc == 'unknown': pc = FALLBACK_CONTAINER
    return p, pd, pc, False

# ── form_configs.json ─────────────────────────────────────────────
configs = []
for firm in firms:
    p, pd, pc, _ = get_colors(firm)
    heading_css, _ = resolve_font(firm.get('heading_font'))
    configs.append({
        'firm_slug': firm['firm_slug'],
        'form_name': 'Estate Planning',
        'brand': {
            'firmName': firm['firm_name'],
            'heroTitle': "Let's get your estate plan started.",
            'heroSubtitle': 'Complete this intake form and our team will be in touch.',
            'logoUrl': clean_logo_url(firm.get('logo_url')),
            'logoAlt': firm['firm_name'],
            'webhookUrl': 'PLACEHOLDER — set before going live',
            'colors': {
                'primary': p,
                'primaryDark': pd,
                'primaryContainer': pc,
...
with open(os.path.join(OUT_DIR, 'form_configs.json'), 'w') as f:
...
def make_html(firm_name, primary, primary_dark, primary_container, heading_gf=None):
    ...
    gf_families = 'family=Inter:wght@400;500;600;700&family=Comfortaa:wght@400;600;700&family=Source+Sans+3:wght@400;500;600;700'
...
body{{background:#f0ead8;font-family:'Source Sans 3',sans-serif;display:flex;justify-content:center;align-items:flex-start;min-height:100vh;padding:24px 12px;}}
#donna-app{{
...
/* preview layout */
#donna-app .preview-page{{display:flex;flex-direction:column;height:100%;overflow:hidden;background:#f0ead8;border-radius:16px;border:1px solid var(--cream-border);font-family:'Source Sans 3',sans-serif;}}
...
#donna-app .preview-sidebar-prog-fill{{height:100%;background:var(--firm-primary);border-radius:3px;transition:width 0.3s;}}
...
#donna-app .preview-field input{{width:100%;padding:9px 12px;border:1.5px solid var(--cream-border);border-radius:8px;font-size:0.85rem;outline:none;transition:border-color 0.15s;background:#fdfaf4;color:#1e1209;}}
#donna-app .preview-field input:focus{{border-color:var(--firm-primary);}}
#donna-app .preview-field textarea{{width:100%;padding:9px 12px;border:1.5px solid var(--cream-border);border-radius:8px;font-size:0.85rem;outline:none;resize:vertical;min-height:76px;transition:border-color 0.15s;background:#fdfaf4;color:#1e1209;}}
#donna-app .preview-field textarea:focus{{border-color:var(--firm-primary);}}
#donna-app .preview-field select{{width:100%;padding:9px 12px;border:1.5px solid var(--cream-border);border-radius:8px;font-size:0.85rem;outline:none;background:#fdfaf4;color:#1e1209;cursor:pointer;transition:border-color 0.15s;appearance:none;background-image:url("data:image/svg+xml,%3Csvg width='10' height='6' viewBox='0 0 10 6' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1l4 4 4-4' stroke='%23565e6c' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E");background-repeat:no-repeat;background-position:right 12px center;padding-right:30px;}}
...
function renderPreviewSidebar() {{
  ...
  var seenInPart=0, partPageIdx=0;
  for(var i=0;i<form.pages.length;i++){{
    if(form.pagePartMap[i]===currentPart){{
      if(i===currentPreviewPage) partPageIdx=seenInPart;
      seenInPart++;
    }}
  }}
  var html='';
...
for i, firm in enumerate(firms):
    ...
    rel_path = f'prospects/previews/{slug}.html'
    summary_rows.append((name, slug, p, 'YES' if fallback else 'no', rel_path))
    if fallback:
        fallback_firms.append((name, slug))

    # Batch progress every 10
    if (i + 1) % 10 == 0 or i + 1 == len(firms):
        batch_start = (i // 10) * 10 + 1
        batch_end = i + 1
        batch_slugs = [firms[j]['firm_slug'] for j in range(batch_start - 1, batch_end)]
        print(f'\nBatch {(i//10)+1} ({batch_start}–{batch_end}):')
        for s in batch_slugs:
            print(f'  ✓ previews/{s}.html')

print(f'\n{"="*70}')
print(f'SUMMARY — {len(firms)} unique firms generated')
print(f'{"="*70}')
print(f'{"Firm Name":<45} {"Slug":<40} {"Color":<10} {"Fallback?":<10}')
print('-'*110)
for name, slug, color, fb, path in summary_rows:
    print(f'{name:<45} {slug:<40} {color:<10} {fb:<10}')

print(f'\n{"="*70}')
print(f'FLAGGED — {len(fallback_firms)} firms using fallback colors (need manual color work):')
print(f'{"="*70}')
for name, slug in fallback_firms:
    print(f'  • {name} ({slug})')

print(f'\nDone. {len(firms)} form_configs entries + {len(firms)} preview files written.')
